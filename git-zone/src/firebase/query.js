...
async function getZoneAllocation(civilianData) {
    ...
    const data = {
        ...
        location: civilianData.location, // { latitude: x, longitude: y }
    ...
    const distances = zones.map(zone => {
        const distance = getDistance(
            { latitude: zone.epicenter.latitude, longitude: zone.epicenter.longitude },
    ...
    if (data.ability === 0) {
        ...
        const zone = distances.find(zone => zone.zone.capacity === Math.min(...distances.map(zone => zone.zone.capacity)));
        return zone;
    } else if (data.ability === 1) {
        // return the closest zone with the same category as the civilian
        const zone = distances.find(zone => zone.zone.isFC === data.class);
        return zone;
...
//         getHaversine(getPersonAndNearest(civilian)) <= maxProximity
//         // getHaversine(civilian.data.latitude, civilian.data.longitude, zone.data.latitude, zone.data.longitude) <= maxProximity
//     );
// }
